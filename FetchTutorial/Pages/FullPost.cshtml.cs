...
{
    public class FullPostModel : PageModel
    {
        private readonly ILogger<IndexModel> _logger;
        public List<PostInfo> posts;
        private readonly ICookieService _cookieService;
        private const string Page = nameof(FullPostModel);

        public FullPostModel(ILogger<IndexModel> logger, ICookieService cookieService)
        {
            _logger = logger;
        ...
        public PageResult OnPostFullPage()
        {
            // check for existing posts
            if (_cookieService.Exists(Helpers.PostsKey))
            {
            ...
            // add a new post
            posts.Add(new PostInfo { Id = posts.Count + 1, Action = nameof(OnPostFullPage), PostPage = Page });
            var json = JsonSerializer.Serialize(posts);
            _cookieService.Set(Helpers.PostsKey, json, 10);
